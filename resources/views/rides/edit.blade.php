...
<div class="container">
    ...
    <form method="POST" action="{{ route('rides.update', $ride) }}">
        ...
        <select id="vehicle_id" name="vehicle_id">
            @foreach ($vehicles as $vehicle)
                <option value="{{ $vehicle->id }}"
                    {{ old('vehicle_id', $ride->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
        ...
        <input id="costo_por_espacio" name="costo_por_espacio" type="number" step="0.01"
               value="{{ old('costo_por_espacio', $ride->costo_por_espacio) }}" required>

        <label for="espacios_totales">Espacios totales</label>
        <input id="espacios_totales" name="espacios_totales" type="number" min="1"
               value="{{ old('espacios_totales', $ride->espacios_totales) }}" required>
